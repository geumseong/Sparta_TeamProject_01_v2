...
int Goblin::performAction() 
{
    ...
    if (type == "겁쟁이") { return performCowardlyAction(); }
    else if (type == "재벌") { return performRichAction(); }
    else if (type == "건방진") { return performCheekyAction(); }
    else if (type == "사나운") { return performFierceAction(); }
...
{
    std::cout << this->name << "가 몽둥이를 휘두릅니다!" << attack << std::endl;
...
int Goblin::performFierceAction()
{
    ...
    std::cout << name << "가 포효하며 무자비한 공격을 퍼붓습니다! 공격력: " << finalDamage << "!" << std::endl;
...
int Goblin::performRichAction()
{
    if (turnCount == 3) 
    {
        std::cout << this->name << "가 당신을 비웃으며 도망쳤습니다! (도주 성공!)" << std::endl;
        this->health = 0;
        this->isFled = true;
		return 0; // 도주 성공, 데미지 없음
    }
    ...
    {
        std::cout << this->name << "가 거만하게 재력을 과시합니다!" << std::endl;
