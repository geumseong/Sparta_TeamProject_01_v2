    ...
    : Monster(level), type(type), bonusExp(0), bonusGold(0), turnCount(0), isFled(false)
{

	{
		...
		if (type == u8"일반")
		{
			...
			dropItem.push_back(move(Item(u8"고블린의 부러진 뼈", 10, 1, E_Type::Material)));
			dropItem.push_back(move(Item(u8"깨진 투구 조각", 10, 1, E_Type::Material)));
		}
		...
		else if (type == u8"겁쟁이")
		{
			...
			// this->legendaryWeight = 5; // 전설 아이템 드랍 확률
			dropItem.push_back(move(Item(u8"고블린의 얇은 가죽", 5, 1, E_Type::Material)));
			dropItem.push_back(move(Item(u8"고블린의 가죽 조각", 5, 1, E_Type::Material)));
		}
		...
		else if (type == u8"건방진")
		{
			...
			dropItem.push_back(move(Item(u8"고블린의 긴 손톱", 20, 1, E_Type::Material)));
			dropItem.push_back(move(Item(u8"고블린의 녹슨 단검", 20, 1, E_Type::Material)));
		}
		...
		else if (type == u8"사나운")
		{
			...
			dropItem.push_back(move(Item(u8"고블린의 날카로운 이빨", 20, 1, E_Type::Material)));
			dropItem.push_back(move(Item(u8"고블린의 녹슨 대검", 20, 1, E_Type::Material)));
		}
		...
		else if (type == u8"재벌")
		{
			...
			dropItem.push_back(move(Item(u8"고블린의 황금 조각", 50, 1, E_Type::Material)));
			dropItem.push_back(move(Item(u8"금화 주머니", 100, 1, E_Type::Material)));
		}
		//
