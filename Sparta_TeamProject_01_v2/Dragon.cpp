    ...
    : Monster(level), turnCount(0), fireBreathCooldown(0), isEnraged(false) // 턴과 쿨타임 초기화
{
    ...
    // 3. 이름 설정
    this->name = u8"Lv." + std::to_string(this->level) + u8" 드래곤";

    // 4. 보스 전용 특별 드롭 아이템 설정
    dropItem.push_back(Item(u8"드래곤의 비늘", 1000, 1, E_Type::Material));
    dropItem.push_back(Item(u8"드래곤의 뿔", 3000, 1, E_Type::Material));
    dropItem.push_back(Item(u8"드래곤의 발톱", 2000, 1, E_Type::Material));
    dropItem.push_back(Item(u8"드래곤의 눈알", 5000, 1, E_Type::Material));
    dropItem.push_back(Item(u8"드래곤의 날개", 5000, 1, E_Type::Material));
    dropItem.push_back(Item(u8"드래곤의 심장", 10000, 1, E_Type::Material));
...
int Dragon::performAction()
{
    ...
    if (!isEnraged && this->health <= this->maxHealth / 2)
    {
        ...
        std::cout << this->name << u8" - 2페이즈 돌입!" << std::endl;
        system("pause");


        isEnraged = true; // 분노 상태로 변경
        std::cout << name << u8"가 포효하며 형태를 변화합니다! 주변의 공기가 뜨거워집니다!" << std::endl;
        system("pause");
...
int Dragon::normalAttack() {
    std::cout << this->name << u8"이(가) 거대한 발톱으로 공격합니다!" << std::endl;
    return this->attack; // 기본 공격력 반환
}

int Dragon::fireBreathAttack() {
    int finalDamage = static_cast<int>(this->attack * 1.8); // 기본 공격력의 1.8배
    std::cout << this->name << u8"이(가) 모든 것을 태워버릴 듯한 화염 브레스를 내뿜습니다! (강력한 공격!)" << std::endl;
