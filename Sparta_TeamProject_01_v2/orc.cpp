...
{
    std::cout << this->name << u8"가 묵직한 공격을 시도합니다!" << std::endl;
    return this->attack;
}
...
int Orc::performSavageAction()
{
    if (rand() % 100 < 30)
    {
        ...
        std::cout << this->name << u8"가 광기에 휩싸여 무자비한 일격을 날립니다! (강력한 공격!)" << std::endl;
        return finalDamage;
    }
    else
    {
...
int Orc::performBerserkerAction()
{
    if (!isCharging)
    {
        std::cout << this->name << u8"가 돌진을 준비합니다! 다음 공격이 매우 강력해집니다!" << std::endl;
        isCharging = true;
        return 0;
    }
    else
    {
        ...
        std::cout << this->name << u8"가 돌진하며 당신을 맹공격합니다! (매우 강력한 공격!)" << std::endl;
        isCharging = false;
        return finalDamage;
    }
}